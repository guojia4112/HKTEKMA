...
function initSandbox(){
  ...
  drawBackgroundIfMissing('sandboxPlot').then(function(){
    ...
    'https://api.trans-ekma.com/api/hk/xy2vocnox',
    {
      method:'POST',
      headers:{
  ...
  est = await response.json();
}catch(e){
  console.error('xy2vocnox API failed', e);
}
  ...
  const response2 = await fetch(
    'https://api.trans-ekma.com/api/hk/vocnox2xy',
    {
      method:'POST',
      headers:{
        'Content-Type':'application/json'
  ...
  const xy = await response2.json();

  if(xy && xy.ok && isFinite(xy.x) && isFinite(xy.y)){

      var px=xy.x, py=xy.y;

      var predLabel='Pred ('+px.toFixed(2)+', '+py.toFixed(2)+')';
      ...
      if(isoAnn) annotations.push(isoAnn);
  ...
  console.error('vocnox2xy API failed', e2);
